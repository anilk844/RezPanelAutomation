...
public class SwingsProgram {
	...
	public static void main(String args[])
	{
		...
		t1=new JTextField();
		t1.setBounds(160, 25, 100, 30);
		b1=new JButton();
		
		b1.setBounds(270, 25, 100, 30);
		f.add(l1);
		f.add(t1);
		f.add(b1);
	...
	{
	    String column[]={"TestCaseName","Day Count","Adult Count","Child Count","Infant Count","Room Count","Room Name","Meal Plan","Package Count","Package Name","Package Meal Plan","Package Type","Ticket Count","Ticket Name","AddOnsCount","AddOnsName","Compare Total Amount with Booking Summary Page","Comapre Booking Summary amount with voucher Amount","Status"};        
		for(int i=0;i<a.length;i=i+16)
		{
			Object data[][]=new Object[1][19];
			try
			{
		String TestCaseName=(String) a[i][15];
		data[0][0]=TestCaseName;
		int DayCount=(int) a[i][0];
		data[0][1]=DayCount;
		int AdultCount=(int) a[i][1];
		data[0][2]=AdultCount;
		int ChildCount=(int)a[i][2];
		data[0][3]=ChildCount;
		int InfantCount=(int)a[i][3];
		data[0][4]=InfantCount;
		int RoomCount=(int)a[i][4];
		data[0][5]=RoomCount;
		String RoomName=(String) a[i][5];
		data[0][6]=RoomName;
		String MealPlan=(String) a[i][6];
		data[0][7]=MealPlan;
		int PackageCount=(int) a[i][7];
		data[0][8]=PackageCount;
		String PackageName=(String) a[i][8];
		data[0][9]=PackageName;
		String PackageMealPlanName=(String) a[i][9];
		data[0][10]=PackageMealPlanName;
		String PackageType=(String) a[i][14];
		data[0][11]=PackageType;
		int TicketCount=(int) a[i][12];
		data[0][12]=TicketCount;
		String TicketName=(String) a[i][13];
		data[0][13]=TicketName;
		int AddOnsCount=(int) a[i][10];
		data[0][14]=AddOnsCount;
		String AddOnsName=(String) a[i][11];
		data[0][15]=AddOnsName;
	   ...
	   //child count selection
	    ...
	    if(totalAmount==totalAMTValue)
	    {
	    	...
	    	data[0][16]="Total Amount :-"+totalAmount+" and Booking Summary Page Total Amount :-"+totalAMTValue+" is Matched";
	    ...
	    else
	    {
	    	...
	    	data[0][16]="Total Amount :-"+totalAmount+"and Booking Summary Page Total Amount :-"+totalAMTValue+" is not Matched";
	    ...
	    {
	    	data[0][17]="Booking Summary Page Total :-"+amt+"and Voucher Page Amount :-"+BooingtotalAmt+" is Matched ";
	    ...
	    {
	    	data[0][17]="Booking Summary Page Total :-"+amt+"and Voucher Page Amount :-"+BooingtotalAmt+" is not Matched ";
	    	System.out.println("amount mismatch");
